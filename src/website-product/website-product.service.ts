...
export class WebsiteProductService {
  ...
  async handleWebsiteProductDetails(
    ...
    for (const [key, fileArray] of Object.entries(photos)) {
      ...
      const storageFile = this.storage
      ...
      filePaths[key as keyof typeof photos] =
        `https://storage.googleapis.com/${this.bucketName}/${uniqueFileName}`;
    }

<<<<<<< HEAD
    try {
      if (dto.id) {
        console.log(filePaths, "111111++++");
...
>>>>>>> 1afe984004b005c2ec7b6a37d6428a0cc509e73a
    }
  }

<<<<<<< HEAD
  async createWebsiteProductDetails(
    dto: WebsiteProductDto,
    ...
    try {
      ...
      return new CommonResponse(
        true,
        201,
        "WebsiteProduct created successfully",
        entity
      );
    } catch (error) {
      throw new ErrorResponse(500, error.message);
    ...
    async createWebsiteProductDetails(dto: WebsiteProductDto, filePaths?: Record<string, string | null>,): Promise<CommonResponse> {
        try {
            ...
            if (filePaths) {
                ...
                entity.productIcon = filePaths.productIcon
                entity.chooseImage = filePaths.chooseImage
                entity.solutionImage = filePaths.solutionImage
            }
            console.log(entity, "???????")
            await this.websiteProductRepository.insert(entity);
...
<<<<<<< HEAD
      ...
      console.log(updatedEntity, "kkkkkkkk");
      await this.websiteProductRepository.save(updatedEntity);
      return new CommonResponse(
        true,
        200,
        "WebsiteProduct updated successfully",
        updatedEntity
      );
    } catch (error) {
      throw new ErrorResponse(500, error.message);
=======
            ...
            const updatedEntity = {
                ...
                productIcon: filePaths.productIcon ?? existing.productIcon,
                chooseImage: filePaths.chooseImage ?? existing.chooseImage,
                solutionImage: filePaths.solutionImage ?? existing.solutionImage
            };
...
>>>>>>> 1afe984004b005c2ec7b6a37d6428a0cc509e73a
  ...
  async deleteWebsiteProductDetails(
    dto: WebsiteProductIdDto
  ): Promise<CommonResponse> {
    try {
      const existing = await this.websiteProductRepository.findOne({
        where: {
      ...
      return new CommonResponse(
        true,
        200,
        "WebsiteProduct deleted successfully"
      );
    } catch (error) {
      throw new ErrorResponse(500, error.message);
    }
  ...
  async getWebsiteProductDetailsById(
    req: WebsiteProductIdDto
  ): Promise<CommonResponse> {
    try {
      const item = await this.websiteProductRepository.findOne({
        ...
        relations: ["device", "amenities", "application", "productApp", "Blog"],
      });
      ...
      return new CommonResponse(
        true,
        200,
        "WebsiteProduct fetched successfully",
        item
      );
    } catch (error) {
      throw new ErrorResponse(500, error.message);
  ...
  async getWebsiteProductDetails(req: CommonReq): Promise<CommonResponse> {
    try {
      const items = await this.websiteProductRepository
        ...
        .where(
          "product.companyCode = :companyCode AND product.unitCode = :unitCode",
          {
            companyCode: req.companyCode,
            unitCode: req.unitCode,
          }
  ...
  async getWebsiteProductDropdown(): Promise<CommonResponse> {
    try {
      const data = await this.websiteProductRepository.find({
        select: ["id", "name"],
      });
      if (data.length) {
